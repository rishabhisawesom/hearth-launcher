...
private final class NetflixKeyContext {
...
struct NetflixExperienceView: View {
    let app: CuratedApp
    let onClose: () -> Void

    @Environment(\.hearthPalette) private var palette
    @State private var store = NetflixCatalogStore()
    @State private var harvestHandler = NetflixCatalogHarvester.makeMessageHandler { _, _ in }
    @State private var playbackURL: URL?
    @State private var webContentFocused = true
    @State private var browseFocusSeed = 0
    @State private var forceWebShell = false
    @State private var keyContext = NetflixKeyContext()
    ...
    var body: some View {
        ...
        .onAppear {
            harvestHandler.onPayload = { jsonString, sourceURL in
                Task { @MainActor in
        ...
        ZStack(alignment: .topLeading) {
            ...
            if playbackURL == nil, store.showsHarvestWebView, let harvestURL {
                StreamingWebView(
                    ...
                    additionalUserScripts: [NetflixCatalogHarvester.userScript],
                    scriptMessageHandlers: [(NetflixCatalogHarvester.messageHandlerName, harvestHandler)],
    ...
    private var nativeBrowseContent: some View {
        VStack(spacing: 0) {
            ...
            NetflixBrowseView(catalog: store.catalog, focusSeed: browseFocusSeed) { tile in
    ...
    private var harvestStatusBanner: some View {
        VStack(alignment: .leading, spacing: HearthSpacing.grid) {
            ...
            if store.phase == .needsLogin {
                Text("Sign in below. Catalog rows appear automatically after Netflix loads.")
                    .font(HearthTypography.body)
                    .foregroundStyle(palette.textSecondary)
    ...
    private var statusMessage: String {
        switch store.phase {
        case .loading:
            store.responseCount > 0 ? "Parsing catalog…" : "Loading Netflix…"
        case .needsLogin:
            "Sign in to Netflix"
        case .ready:
            "Loading browse…"
    ...
    private func installKeyMonitor() {
        ...
        keyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
            // nativeBrowse: arrows/Return/Esc go to NetflixBrowseView (.onKeyPress)
